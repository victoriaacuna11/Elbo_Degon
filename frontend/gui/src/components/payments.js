...
class Payments extends React.Component {
  ...
  getBillID = (paymentID, payment_bill_relationsips) => {
    ...
    const billID = payment_bill_relationsips.filter(
  ...
  getClientID = (billID, bills) => {
    // Una vez que sabe cuál es el ID de la factura asociada al pago, busca el id del cliente asociado a ella.
    const clientID = bills.filter(item => item.id == billID)[0].client;
    return clientID;
  };

  getClientCI = (clientID, clients) => {
    // Con el id del cliente, busca los datos del cliente para encontrar la cédula.
    const ci = clients.filter(item => item.id == clientID)[0].ci;
    return ci;
  };

  getTotal = (taxes, subtotal) => {
    const taxSelected = taxes.filter(t => t.is_Active == true)[0].tax;
  ...
  getSubtotal = (billID, bills) => {
    console.log(billID);
    return bills.filter(item => item.id == billID)[0].subtotal;
  };
                    ...
                    <a href={`pay/${item.id}`}>
                      ...
                      {this.getClientCI(
                        this.getClientID(
                          this.getBillID(
                            item.id,
                            this.state.payment_bill_relationships
                          ),
                          this.state.bills
                        ),
                        this.state.clients
